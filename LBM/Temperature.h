...
__global__ void InitialTempG()
{
    ...
    Temp[x][y] = .50f; // Initial Temperature
    //	distributions
...
__global__ void BoundaryEastTemp()
{
    int y = threadIdx.y + blockIdx.y * blockDim.y;
    int x = Nx - 1;
    float TempConst = (1.0 - 1.0f * y / Ny) * .2 / 18.0f; //temperature from 0 to 0.2
    float alphaT = 1.;
    ...
    Tin[x][y].fW = (1. - alphaT) * Tin[x][y].fE + alphaT * (4.0f * TempConst - Tin[x][y].fE);
    Tin[x][y].fSW = (1. - alphaT) * Tin[x][y].fNE + alphaT * (TempConst - Tin[x][y].fNE);
    Tin[x][y].fNW = (1. - alphaT) * Tin[x][y].fSE + alphaT * (TempConst - Tin[x][y].fSE);
    return;
}
...
__global__ void BoundaryWestTemp()
{
    ...
    float TempConst = (1.0 - 1.0f * y / Ny) * 0.2 / 18.f;//temperature from 0 to 0.2
    float alphaT = 1.;
    ...
    Tin[0][y].fE = (1. - alphaT) * Tin[0][y].fW + alphaT * (4.0f * TempConst - Tin[0][y].fW);
    Tin[0][y].fNE = (1. - alphaT) * Tin[0][y].fSW + alphaT * (TempConst - Tin[0][y].fSW);
    Tin[0][y].fSE = (1. - alphaT) * Tin[0][y].fNW + alphaT * (TempConst - Tin[0][y].fNW);
    return;
    Temp[x][y] = (Tin[0][y].fC + Tin[0][y].fN + Tin[0][y].fS + 2 * (Tin[0][y].fW + Tin[0][y].fNW + Tin[0][y].fSW)) / (1 - AtmosVx[0][y]);
    Tin[0][y].fE = Tin[0][y].fW + 2. / 3. * Temp[0][y] * AtmosVx[0][y];
    Tin[0][y].fNE = Tin[0][y].fNW + Temp[0][y] * AtmosVx[0][y] / 6.;
    Tin[0][y].fSE = Tin[0][y].fSW + Temp[0][y] * AtmosVx[0][y] / 6.;
    return;
}
...
__global__ void BoundarySouthTemp()
{
    ...
    float TempConst = (1.0f * x / Nx) * 1.0 / 18.0f; // temperature from 0 to 1
    float alphaT = 0.;
    ...
    Tin[x][0].fN = (1. - alphaT) * Tin[x][0].fS + alphaT * (4.0f * TempConst - Tin[x][0].fS);
    Tin[x][0].fNW = (1. - alphaT) * Tin[x][0].fSE + alphaT * (TempConst - Tin[x][0].fSE);
    Tin[x][0].fNE = (1. - alphaT) * Tin[x][0].fSW + alphaT * (TempConst - Tin[x][0].fSW);
    return;
}
...
__global__ void BoundaryNordTemp()
{
    ...
    Tin[x][y].fS = (1. - alphaT) * Tin[x][y].fN + alphaT * (4.0f * TempConst - Tin[x][y].fN);
    Tin[x][y].fSW = (1. - alphaT) * Tin[x][y].fNE + alphaT * (TempConst - Tin[x][y].fNE);
    Tin[x][y].fSE = (1. - alphaT) * Tin[x][y].fNW + alphaT * (TempConst - Tin[x][y].fNW);
    // return
    Temp[x][y] = (Tin[x][y].fC + Tin[x][y].fE + Tin[x][y].fW + 2 * (Tin[x][y].fN + Tin[x][y].fNW + Tin[x][y].fNE)) / (1 + AtmosVy[x][y]);
    Tin[x][y].fS = Tin[x][y].fN - 2. / 3. * Temp[x][y] * AtmosVy[x][y];
    Tin[x][y].fSE = Tin[x][y].fNE - Temp[x][y] * AtmosVy[x][y] / 6.;
    Tin[x][y].fSW = Tin[x][y].fNW - Temp[x][y] * AtmosVy[x][y] / 6.;
    return;
}
